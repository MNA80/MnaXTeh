...
if not windLoadOk or not WindUI then
    warn("[MNA] CRITICAL: WindUI 1.6.63 failed to load: " .. tostring(windLoadErr))
    ...
    emergencyLabel.Text = "[MNA HUB] WindUI 1.6.63 failed to load.\nError: " .. tostring(windLoadErr):sub(1, 100)
...
pcall(function()
    if WindUI.Creator and WindUI.Creator.AddIcons then
        WindUI.Creator.AddIcons("MNA HUB",)
    end
end)
...
local winOk, winErr = pcall(function()
    Window = WindUI:CreateWindow({
        ...
        OpenButton = {
            Enabled = true, Title = "MNA HUB", Icon = "atom",
...
if not winOk or not Window then
    warn("[MNA] Window creation failed: " .. tostring(winErr))
    return
end

local function SafeCreate(creationFunc, errorPrefix)
    local ok, result = pcall(creationFunc)
    if not ok then warn("[MNA] " .. (errorPrefix or "UI Error") .. ": " .. tostring(result)); return nil end
    return result
end
-- TABS
...
local ExclusiveTab = SafeCreate(function() return Window:Tab({ Title = "Fishing", Icon = "fish" })         end, "ExclusiveTab")
local CraftAbilityTab = SafeCreate(function() return Window:Tab({ Title = "Craf", Icon = "wrench" }) end, "CraftAbilltyTab")
...
local function NotifyError(title, text)
    pcall(function() if WindUI and WindUI.Notify then WindUI:Notify({ Title = "[ERROR] "  .. title, Content = text, Duration = 3, Icon = "x" }) end end)
end
...
-- MEGALODON QUEST
    ...
    print("[MNA] Loaded: " .. loaded .. " | Failed: " .. failed)
...
-- BARU
    ...
    YTTA = {Active = false, Settings = {QHDelay = 2.8671}, NotifCount = 3, NotifDelay = 0.1},
}
...
local _logPrefix = "[MNA_" .. _instanceId .. "]"
...
-- REPLION HOOK (RAINBOW/GOLDEN) - NEW HOOKFUNCTION APPROACH
pcall(function()
    ...
    -- Hook directly into replion Set event connections
        ...
        local old; old = hookfunction(Connection.Function, function(...)
            ...
            if type(Args[2]) == "table" then
                ...
                local function RunNaturalUpdate(updateType)
                    task.spawn(function()
                        -- Fire twice untuk effect (visual catch jadi double)
                        for i = 1, 2 do
                ...
                -- ALWAYS update (amblatant mode dan normal mode)
...
local function safeFire(func)
    task.spawn(function()
        ...
        if not ok then warn("[MNA] safeFire error: " .. tostring(err)) end
    end)
end
    ...
    pcall(function()
        humanoid.AnimationPlayed:Connect(function(track)
            ...
            if animName:find("fishcaught") or animName:find("caught") or animName:find("reel") then
                -- ONLY speed up animations when Quantum Fishing Beta is active
...
local function triggerRainbowGoldenUpdate(notifData)
    ...
    -- Cek dari metadata index 2 (paling akurat)
    local meta = notifData[2]
    if type(meta) == "table" then
        local variant = tostring(meta.VariantId or ""):lower()
        local mutation = tostring(meta.Mutation or ""):lower()
        if variant:find("rainbow") or mutation:find("rainbow") then isRainbow = true end
        if variant:find("gold") or mutation:find("gold") or meta.Shiny == true then isGolden = true end
    end

    -- Fallback scan semua arg
    ...
    pcall(function()
        local replionFolder = ReplicatedStorage:FindFirstChild("Packages")
        if not replionFolder then return end
        local idx2 = replionFolder:FindFirstChild("_Index")
        if not idx2 then return end
        for _, child in ipairs(idx2:GetChildren()) do
            if child.Name:find("ytrev_replion") then
                local replionMod = child:FindFirstChild("replion")
                if replionMod then
                    local remotes = replionMod:FindFirstChild("Remotes")
                    if remotes then replionSetEvent = remotes:FindFirstChild("Set"); break end
                end
            end
        end
    end)

    if not replionSetEvent or not LocalPlayer then return end

    -- Rainbow nambah 1x per catch (bukan looping)
    if isRainbow then
        rainbowCount = rainbowCount + 1
        if rainbowCount > 40 then rainbowCount = 0 end
        pcall(function()
            FireLocalEvent(replionSetEvent, LocalPlayer, {"Modifiers", "Rainbow"}, rainbowCount)
        end)
    end

    -- Golden nambah 1x per catch
    if isGolden then
        goldenCount = goldenCount + 1
        if goldenCount > 10 then goldenCount = 0 end
        pcall(function()
            FireLocalEvent(replionSetEvent, LocalPlayer, {"Modifiers", "Golden"}, goldenCount)
        end)
    end

    -- Fish inventory update 1x
    fishCount = fishCount + 1
    pcall(function()
        FireLocalEvent(replionSetEvent, LocalPlayer, {"InventoryNotifications", "Fish"}, fishCount)
    end)
end
...
local function replayAmblatantNotif()
    task.spawn(function()
        ...
        if xr_notif and #lastValidFishNotif > 0 then
            for i = 1, Config.YTTA.NotifCount do
                -- Fire notification with different fish from history
                local notifData = lastValidFishNotif
                if #_fishNotifHistory > 0 then
                    local historyIdx = ((i - 1) % #_fishNotifHistory) + 1
                    notifData = _fishNotifHistory[historyIdx]
                    local _fishNotifHistoryKeys = {} -- track unique key per ikan
                    local _lastFiredNotifKey = nil   -- track notif terakhir yang dikirim
                end
                pcall(function() FireLocalEvent(xr_notif, unpack(notifData)) end)
                
                -- Update inventory count untuk visual fish
                updateReplionInventory(notifData)
                
                -- Fire visual and catch for each notification (visual changes each time)
                if xr_notif and #lastValidFishNotif > 0 then
    -- Visual + rainbow hanya SEKALI
    if xr_visual and #lastValidCaughtVisual > 0 then
        pcall(function() FireLocalEvent(xr_visual, unpack(lastValidCaughtVisual)) end)
    end
    pcall(function() triggerRainbowGoldenUpdate(lastValidFishNotif) end)

    -- Kumpulkan notif unik dari history (tidak ada yang sama)
    local usedKeys = {}
    local uniqueNotifs = {}
    -- Tambahkan lastValidFishNotif dulu sebagai index 1
    local baseKey = GetNotifUniqueKey(lastValidFishNotif)
    usedKeys[baseKey] = true
    table.insert(uniqueNotifs, lastValidFishNotif)

    -- Tambahkan dari history yang berbeda
    for i = #_fishNotifHistory, 1, -1 do
        if #uniqueNotifs >= Config.YTTA.NotifCount then break end
        local k = _fishNotifHistoryKeys[i] or GetNotifUniqueKey(_fishNotifHistory[i])
        if not usedKeys[k] then
            usedKeys[k] = true
            table.insert(uniqueNotifs, _fishNotifHistory[i])
        end
    end

    -- Fire sejumlah NotifCount, pakai unique notif
    for i = 1, Config.YTTA.NotifCount do
        local notifData = uniqueNotifs[i] or uniqueNotifs[#uniqueNotifs] or lastValidFishNotif
        pcall(function() FireLocalEvent(xr_notif, unpack(notifData)) end)
    ...
    for i = 1, Config.YTTA.NotifCount do
        ...
        if #_fishNotifHistory > 0 then
            local historyIdx = ((i - 1) % #_fishNotifHistory) + 1
            notifData = _fishNotifHistory[historyIdx]
                local function GetNotifUniqueKey(notifData)
    if not notifData or #notifData == 0 then return "unknown" end
    local id = tostring(notifData[1] or "")
    local meta = notifData[2]
    local variant = ""
    local weight = ""
    if type(meta) == "table" then
        variant = tostring(meta.VariantId or meta.Mutation or "")
        weight  = tostring(meta.Weight or "")
    end
    return id .. "|" .. variant .. "|" .. weight
end

local function AddToFishHistory(notifData)
    if not notifData or #notifData == 0 then return end
    local key = GetNotifUniqueKey(notifData)
    -- Cek apakah sudah ada di history dengan key sama
    -- Kalau sama persis dengan lastFired, skip untuk hindari double
    if _lastFiredNotifKey == key then return end
    -- Tambahkan ke history
    table.insert(_fishNotifHistory, AddToFishHistory(args)
    table.insert(_fishNotifHistoryKeys, key)
    if #_fishNotifHistory > _maxFishHistory then
        table.remove(_fishNotifHistory, 1)
        table.remove(_fishNotifHistoryKeys, 1)
    end
    _lastFiredNotifKey = key
end
        end
        pcall(function() FireLocalEvent(xr_notif, unpack(notifData)) end)
        if xr_caught and #lastValidFishCaught > 0 then
...
local function ub_loop()
    while Config.UB.Active do
        local ok, err = pcall(function()
            ...
            task.wait(baseWait)
            needCast = false
            -- FIRE Charge and RequestMinigame with minimal delay for speed
            safeFire(function()
            ...
            -- Reduced wait between charge and minigame (0.1 -> 0.05, antiOKOK 15-25ms -> 8-15ms)
            if Config.antiOKOK then task.wait(math.random(8, 15) / 100) else task.wait(0.02) end
            safeFire(function()
            ...
            -- Use QHDelay if YTTA is active, otherwise use CompleteDelay
            ...
            if not skip then
                ...
                if Config.amblatant then
                    isCaught = false
                    local waited = 0
                    -- Faster polling for isCaught (0.05 -> 0.03, timeout 0.8 -> 0.5)
                    while not isCaught and waited < 0.25 do task.wait(0.01); waited = waited + 0.01 end
                    if isCaught then
                        isCaught = false
                        ...
                        if #lastValidFishNotif > 0 then
                            task.spawn(function()
                                ...
                                local xr_notif = Events.fishNotif
                                if xr_caught and #lastValidFishCaught > 0 then pcall(function() FireLocalEvent(xr_caught, unpack(lastValidFishCaught)) end) end
                                -- Faster notification replay (0.03 -> 0.01)
                                task.wait(0.0005)
                                 if xr_notif and #lastValidFishNotif > 0 then
    -- Visual + rainbow hanya SEKALI
    if xr_visual and #lastValidCaughtVisual > 0 then
    ...
    for i = 1, Config.YTTA.NotifCount do
        ...
        -- Faster error recovery (0.1 -> 0.05)
        if not ok then warn("[MNA] UB error: " .. tostring(err)); task.wait(0.05) end
    end
end
...
local function UB_start()
    ...
    NotifySuccess("Fast FISHING", "Aktif!")
end
...
local function UB_stop()
    ...
    NotifyWarning("Fast FISHING", "Dimatikan.")
end
...
-- Separate handler for Quantum YTTA (no animation speed-up)
local function onToggleYTTA(value)
    ...
    if value then
        ...
        Config.UB.Stats.startTime = tick()
        Tasks.ubtask = task.spawn(ub_loop)
        NotifySuccess("amBlantat YTTA", "Aktif!")
    else
        Config.UB.Active = false; _G.NotifQueue = {}; _G.NotifActive = 0
        ...
        task.wait(0.3)
        if Tasks.ubtask then pcall(function() task.cancel(Tasks.ubtask) end); Tasks.ubtask = nil end
        NotifyWarning("amBlantat YTTA", "Dimatikan.")
    end
end
...
local function CreateQuantumPanel()
    ...
    gui.Name = "MnaHubPanel"; gui.IgnoreGuiInset = true; gui.ResetOnSpawn = false; gui.Enabled = true; gui.Parent = CoreGui
...
local function RunAutoAtlantisMachine()
    ...
    AtlantisConfig.MachineThread = task.spawn(function()
        while AtlantisConfig.AutoAtlantisMachine and AtlantisConfig.IsRunning do
            ...
            if not ok then
                warn("[MNA] Atlantis error: " .. tostring(err))
            end
...
local function RunAutoHatch()
    ...
    PetEggConfig.AutoHatchThread = task.spawn(function()
        while PetEggConfig.AutoHatch do
            ...
            if not ok then
                warn("[MNA] AutoHatch error: " .. tostring(err))
            end
            task.wait(2)
        end
    end)
...
local function RunAutoBuyFounderEgg()
    ...
    FounderEggConfig.Thread = task.spawn(function()
        while FounderEggConfig.AutoBuy and FounderEggConfig.IsRunning do
            ...
            if not ok then
                warn("[MNA] FounderEgg error: " .. tostring(err))
            end
            task.wait(5)
        end
...
local function RunAutoCrafting()
    ...
    Tasks.CraftingThread = task.spawn(function()
        while Config.AutoCrafting do
            ...
            if not ok then warn("[MNA] AutoCrafting error: " .. tostring(err)) end
...
local function ApplyCustomName(name)
    if not name or name == "" then NotifyError("Custom Name"); return end
    ...
    NotifySuccess("Custom Name" .. name)
end
...
local function LoadTradeRemotes()
    -- Try multiple remote name variations for better compatibility
    TradeRemotes.InitiateTrade = GetServerRemote("RF/InitiateTrade") or GetServerRemote("RF/Trading/InitiateTrade")
    TradeRemotes.CanSendTrade = GetServerRemote("RF/CanSendTrade") or GetServerRemote("RF/Trading/CanSendTrade")
    TradeRemotes.AddItem = GetServerRemote("RF/Trading/AddItem") or GetServerRemote("RF/Trading/AddItemToTrade")
    TradeRemotes.RemoveItem = GetServerRemote("RF/Trading/RemoveItem") or GetServerRemote("RF/Trading/RemoveItemFromTrade")
    TradeRemotes.SetTokens = GetServerRemote("RF/Trading/SetTokens") or GetServerRemote("RF/Trading/SetCoins")
    TradeRemotes.SetReady = GetServerRemote("RF/Trading/SetReady") or GetServerRemote("RF/Trading/MarkReady")
    TradeRemotes.ConfirmTrade = GetServerRemote("RF/Trading/ConfirmTrade") or GetServerRemote("RF/Trading/CompleteTrade")
    ...
    -- Log loaded remotes for debugging
    local loaded, failed = 0, 0
    for key, remote in pairs(TradeRemotes) do
        if remote then loaded = loaded + 1 else failed = failed + 1; warn("[QH] Trade Remote gagal: " .. key) end
    end
    print("[MNA] Trade Remotes: " .. loaded .. " loaded, " .. failed .. " failed")
end
...
local function SetupAutoAcceptTrade()
    ...
    if not TradeRemotes.TradeOfferReceived then 
        NotifyWarning("Trade", "TradeOfferReceived event tidak ditemukan!")
        return 
    end
    
    if not TradeRemotes.TradeOfferReceived:IsA("RemoteEvent") then 
        NotifyWarning("Trade", "TradeOfferReceived bukan RemoteEvent!")
        return 
    end
    ...
    pcall(function()
        autoAcceptConn = TradeRemotes.TradeOfferReceived.OnClientEvent:Connect(function(...)
            ...
            local args = {...}
            pcall(function() 
                if TradeRemotes.AcceptTradeOffer then 
                    TradeRemotes.AcceptTradeOffer:InvokeServer(unpack(args))
                    NotifySuccess("Auto Accept", "Trade diterima!") 
                end 
            end)
        end)
        NotifySuccess("Auto Accept", "Listener aktif!")
    end)
end
...
local function ExecuteTrade(targetPlayer, itemUUIDs, coinAmount)
    if not targetPlayer then NotifyError("Trade", "Target player kosong!"); return false end
    
    -- Convert string name to player object if needed
    if type(targetPlayer) == "string" then
        local player = Players:FindFirstChild(targetPlayer)
        if not player then NotifyError("Trade", "Player '" .. targetPlayer .. "' tidak ditemukan!"); return false end
        targetPlayer = player
    end
    
    local success = false
    pcall(function()
        if not TradeRemotes.InitiateTrade then NotifyError("Trade", "InitiateTrade remote tidak ditemukan!"); return end
        
        -- Check if we can send trade
        local canTrade = true
        if TradeRemotes.CanSendTrade then 
            local canTradeOk, canTradeResult = pcall(function() 
                return TradeRemotes.CanSendTrade:InvokeServer(targetPlayer.Name or tostring(targetPlayer)) 
            end)
            if canTradeOk then canTrade = canTradeResult else canTrade = true end -- Allow trade even if check fails
        end
        
        if not canTrade then 
            NotifyWarning("Trade", "Tidak bisa trade sekarang!")
            return 
        end
        
        -- Initiate trade with player
        pcall(function() TradeRemotes.InitiateTrade:InvokeServer(targetPlayer.Name or tostring(targetPlayer)) end)
        task.wait(1.5)
        
        -- Add items to trade
        if itemUUIDs and #itemUUIDs > 0 then 
            for _, uuid in ipairs(itemUUIDs) do 
                if TradeRemotes.AddItem then 
                    pcall(function() TradeRemotes.AddItem:InvokeServer(uuid) end)
                    task.wait(0.2) 
                end
            end 
            task.wait(0.5)
        ...
        if coinAmount and coinAmount > 0 then 
            if TradeRemotes.SetTokens then 
                pcall(function() TradeRemotes.SetTokens:InvokeServer(coinAmount) end)
                task.wait(0.3)
            end 
        end
        ...
        -- Mark as ready
        task.wait(0.5)
        if TradeRemotes.SetReady then 
            pcall(function() TradeRemotes.SetReady:InvokeServer(true) end) 
            task.wait(0.5)
        end
        
        -- Confirm trade
        if TradeRemotes.ConfirmTrade then 
            pcall(function() TradeRemotes.ConfirmTrade:InvokeServer() end) 
        end
...
local function StartTradeEnchantStone()
    if not TradeConfig.SelectedTargetPlayer or TradeConfig.SelectedTargetPlayer == "Tidak ada player lain" then 
        NotifyError("Trade", "Pilih target player dulu!"); 
        TradeConfig.TradeEnchantStone = false
        return 
    end
    
    local targetPlayer = Players:FindFirstChild(TradeConfig.SelectedTargetPlayer)
    if not targetPlayer then 
        NotifyError("Trade", "Target player tidak ditemukan!"); 
        TradeConfig.TradeEnchantStone = false
        return 
    end
        ...
        while TradeConfig.TradeEnchantStone do
            local ok, err = pcall(function()
                ...
                if not stones or #stones == 0 then 
                    NotifyWarning("Trade", "Tidak ada enchant stone!"); 
                    task.wait(5)
                    return 
                end
                ...
                for _, stone in ipairs(stones) do 
                    if TradeConfig.SelectedStoneType == "All" or stone.Type == TradeConfig.SelectedStoneType then 
                        table.insert(targetStones, stone.UUID) 
                    end 
                end
                
                if #targetStones < TradeConfig.EnchantStoneAmount then 
                    NotifyWarning("Trade", "Stone tidak cukup! (" .. #targetStones .. "/" .. TradeConfig.EnchantStoneAmount .. ")"); 
                    task.wait(5)
                    return 
                end
                
                local uuids = {}
                for i = 1, math.min(TradeConfig.EnchantStoneAmount, #targetStones) do 
                    table.insert(uuids, targetStones[i]) 
                end
                
                local success = ExecuteTrade(targetPlayer, uuids, 0)
                if success then 
                    tradeCount = tradeCount + 1
                    NotifySuccess("Trade", "Enchant stone trade #" .. tradeCount .. " selesai!") 
                else 
                    NotifyError("Trade", "ExecuteTrade gagal!") 
                end
            end)
            
            if not ok then 
                warn("[MNA] TradeEnchantStone error: " .. tostring(err)) 
                NotifyError("Trade EnchantStone", "Error: " .. tostring(err):sub(1, 50))
            end
...
local function StartTradeByRarity()
    if not TradeConfig.SelectedTargetPlayer or TradeConfig.SelectedTargetPlayer == "Tidak ada player lain" then 
        NotifyError("Trade", "Pilih target player dulu!"); 
        TradeConfig.TradeByRarity = false
        return 
    end
        ...
        while TradeConfig.TradeByRarity do
            local ok, err = pcall(function()
                ...
                if not items or #items == 0 then 
                    NotifyWarning("Trade", "Inventory kosong!"); 
                    task.wait(5)
                    return 
                end
                ...
                for _, item in ipairs(items) do 
                    if string.lower(item.Rarity) == string.lower(TradeConfig.SelectedTradeRarity) then 
                        table.insert(targetItems, item.UUID) 
                    end 
                end
                
                if #targetItems < TradeConfig.TradeRarityAmount then 
                    NotifyWarning("Trade", "Ikan " .. TradeConfig.SelectedTradeRarity .. " tidak cukup! (" .. #targetItems .. "/" .. TradeConfig.TradeRarityAmount .. ")"); 
                    task.wait(5)
                    return 
                end
                
                local uuids = {}
                for i = 1, math.min(TradeConfig.TradeRarityAmount, #targetItems) do 
                    table.insert(uuids, targetItems[i]) 
                end
                
                local success = ExecuteTrade(targetPlayer, uuids, 0)
                if success then 
                    tradeCount = tradeCount + 1
                    NotifySuccess("Trade", "Trade " .. TradeConfig.SelectedTradeRarity .. " trade #" .. tradeCount .. " selesai!") 
                else 
                    NotifyError("Trade", "ExecuteTrade gagal!") 
                end
            end)
            ...
            if not ok then 
                warn("[MNA] TradeByRarity error: " .. tostring(err)) 
                NotifyError("Trade ByRarity", "Error: " .. tostring(err):sub(1, 50))
            end
...
local function StartTradeByCoin()
    ...
    if TradeConfig.TargetCoins <= 0 then 
        NotifyError("Trade", "Coins harus > 0!"); 
        TradeConfig.TradeByCoin = false
        return 
    end
    ...
    task.spawn(function()
        ...
        while TradeConfig.TradeByCoin do
            local ok, err = pcall(function()
                local success = ExecuteTrade(targetPlayer, nil, TradeConfig.TargetCoins)
                if success then 
                    tradeCount = tradeCount + 1
                    NotifySuccess("Trade", "Trade " .. TradeConfig.TargetCoins .. " coins trade #" .. tradeCount .. " selesai!") 
                else 
                    NotifyError("Trade", "ExecuteTrade gagal!") 
                end
            end)
            ...
            if not ok then 
                warn("[MNA] TradeByCoin error: " .. tostring(err)) 
                NotifyError("Trade ByCoin", "Error: " .. tostring(err):sub(1, 50))
            end
...
local function StartTradeByName()
    if not TradeConfig.SelectedTargetPlayer or TradeConfig.SelectedTargetPlayer == "Tidak ada player lain" then 
        NotifyError("Trade", "Pilih target player dulu!"); 
        TradeConfig.TradeByName = false
        return 
    end
    
    if not TradeConfig.SelectedTradeItem then 
        NotifyError("Trade", "Pilih item dulu!"); 
        TradeConfig.TradeByName = false
        return 
    end
    ...
    local targetPlayer = Players:FindFirstChild(TradeConfig.SelectedTargetPlayer)
    if not targetPlayer then 
        NotifyError("Trade", "Target tidak ditemukan!"); 
        TradeConfig.TradeByName = false
        return 
    end
    ...
    task.spawn(function()
        ...
        while TradeConfig.TradeByName do
            local ok, err = pcall(function()
                ...
                local targetUUIDs = {}
                if _G.TradeGroupedItems and #_G.TradeGroupedItems > 0 then
                    for _, grp in ipairs(_G.TradeGroupedItems) do
                        if grp.Name == TradeConfig.SelectedTradeItem or grp.Display == TradeConfig.SelectedTradeItem then
                            targetUUIDs = grp.Items or {}
                            break
                        end
                    end
                end
                
                -- fallback: scan raw if cache didn't work
                if #targetUUIDs == 0 then
                    local items = ScanTradeInventory()
                    if items then
                        for _, item in ipairs(items) do
                            if item.Name == TradeConfig.SelectedTradeItem then 
                                table.insert(targetUUIDs, item.UUID) 
                            end
                        end
                    end
                end
                ...
                if #targetUUIDs == 0 then 
                    NotifyWarning("Trade", "Item '" .. TradeConfig.SelectedTradeItem .. "' tidak ditemukan!"); 
                    task.wait(5)
                    return 
                end
                
                if #targetUUIDs < TradeConfig.TradeNameAmount then 
                    NotifyWarning("Trade", "Item tidak cukup! (" .. #targetUUIDs .. "/" .. TradeConfig.TradeNameAmount .. ")"); 
                    task.wait(5)
                    return 
                end
                
                local uuids = {}
                for i = 1, math.min(TradeConfig.TradeNameAmount, #targetUUIDs) do 
                    table.insert(uuids, targetUUIDs[i]) 
                end
                
                local success = ExecuteTrade(targetPlayer, uuids, 0)
                if success then 
                    tradeCount = tradeCount + 1
                    NotifySuccess("Trade", "Trade " .. TradeConfig.SelectedTradeItem .. " trade #" .. tradeCount .. " selesai!")
                else 
                    NotifyError("Trade", "ExecuteTrade gagal!") 
                end
            end)
            ...
            if not ok then 
                warn("TradeByName error: " .. tostring(err)) 
                NotifyError("Trade ByName", "Error: " .. tostring(err):sub(1, 50))
            end
...
if InfoTab then
    pcall(function()
        InfoTab:Section({ Title = "Ancoment Script" })
        InfoTab:Button({
            Title = "Discord Server", Desc = "Klik untuk copy link",
            Callback = function()
                pcall(function() if typeof(setclipboard) == "function" then setclipboard("https://discord.gg/3jpA7XhPFj"); NotifySuccess("Discord", "Link dicopy!") end end)
            end
        })
        InfoTab:Section({ Title = "Map Info" })
        InfoTab:Label({ Title = "Map: " .. (isSupported and supportedMaps["121864768012064"] or mapName) })
        InfoTab:Label({ Title = "Toggle RightShift untuk Show/Hide UI" })
        InfoTab:Label({ Title = "Script FREE - Ketahuan Di Jual Belikan Kita Goreng Di Tempat" })
    end)
end
-- PLAYERS TAB
if PlayersTab then
    pcall(function()
        ...
        PlayersTab:Section({ Title = "Abilities" })
        PlayersTab:Toggle({
        ...
        PlayersTab:Section({ Title = "FreeCam [BETA]" })
        ...
        local freecamModeValues = {
            { Title = "Camera Mov", Icon = "camera" },
            { Title = "Character Mov", Icon = "user" }
        }
        PlayersTab:Dropdown({
            Title = "FreeCam Mode",
            Desc = "Gerakan Layar Camera Sesukanya",
        ...
        PlayersTab:Section({ Title = "Aura Animation" })
        ...
        local function applyLocalAura(auraName)
            ...
            pcall(function()
                ...
                if #instances > 0 then
                    ...
                    NotifyWarning("Aura", "Remote Gagal" .. auraName)
                end
            end)
        end
        ...
        local auraDropdownValues = {{ Title = "Refresh", Icon = "refresh-cw" }}
        auraDropdownRef = PlayersTab:Dropdown({
            Title = "Pilih Aura",
            Desc = "Klik Refresh Terlebih Dahulu",
            Values = auraDropdownValues,
            Value = auraDropdownValues[1],
            Multi = false,
            Callback = function(val)
                if val.Title ~= "Refresh dulu" and val.Title ~= "Tidak ada aura" then
                    _G.SelectedAuraName = val.Title
                    NotifyInfo("Aura", "Selected: " .. val.Title)
                end
            end
        })
        ...
        PlayersTab:Button({
            ...
            Desc = "Scan semua aura di ReplicatedStorage.Assets.Auras",
            Callback = function()
                ...
                if #auras == 0 then
                    ...
                    pcall(function()
                        if auraDropdownRef and auraDropdownRef.Refresh then
                            auraDropdownRef:Refresh(emptyValues, emptyValues[1])
                        end
                    end)
                    return
                end
                ...
                pcall(function()
                    if auraDropdownRef and auraDropdownRef.Refresh then
                        auraDropdownRef:Refresh(newValues, newValues[1])
                    end
                end)
    ...
    pcall(function()
        ...
        MainTab:Button({ Title = "Redeem Code", Callback = function()
            ...
            if Events.RedeemCode then
                ...
                MainTab:Section({ Title = "Enchant" })

        local enchantParagraph = MainTab:Paragraph({
        ...
        MainTab:Dropdown({ Title = "Enchant Stone Type", Values = stoneTypeValues, Value = stoneTypeValues[1], Multi = false, Callback = function(val) _G.SelectedStoneType = val.Title end })
        ...
        MainTab:Dropdown({ Title = "Target Enchant (Evoled)", Values = evolvedEnchantValues, Value = evolvedEnchantValues[1], Multi = false, Callback = function(val) _G.TargetEnchantEvolved = val.Title end })
        ...
        MainTab:Button({ Title = "Teleport to Altar", Callback = function() teleportTo("Enchanting Altar"); NotifySuccess("TP", "Berhasil ke Enchanting Altar!") end })
        ...
        MainTab:Toggle({
            Title = "Open Pirate Chest", Value = false,
            Callback = function(val)
        ...
        MainTab:Toggle({
            Title = "Auto Consume  Crystal", Value = false,
            Callback = function(val)
        ...
        MainTab:Section({ Title = "Fishing Radar (BETA)" })
        MainTab:Toggle({
        ...
        MainTab:Button({ Title = "Teleport Leviathan", Callback = function() local hrp = getHRP(); if hrp then hrp.CFrame = CFrame.new(3474.053,-287.775,3472.634) end end })
        MainTab:Button({ Title = "Teleport Thunderzilla", Callback = function() local hrp = getHRP(); if hrp then hrp.CFrame = CFrame.new(2067.866,2.028,10.831) end end })
        MainTab:Toggle({ Title = "Auto Event", Value = false, Callback = function(val) Config.AutoEvent = val; if val then RunAutoEvent() else if Tasks.AutoEventThread then pcall(function() task.cancel(Tasks.AutoEventThread) end) end end end })
        ...
        MainTab:Toggle({
            Title = "Auto Event Teleport (Opsional)", Value = false,
...
if ExclusiveTab then
    pcall(function()
        ExclusiveTab:Section({ Title = "Auto Fishing (Blantant)" })
        ExclusiveTab:Input({
            Title = "Complete Delay (detik)", Placeholder = "2.7881", Value = "2.7881",
            Callback = function(text)
                local num = tonumber(text); if not num or num < 1 then NotifyError("Delay", "Minimal 0.1 Mili detik!"); return end
                Config.UB.Settings.CompleteDelay = math.clamp(num, 1, 10); NotifySuccess("Delay", "Set ke " .. Config.UB.Settings.CompleteDelay .. "s")
            end
        })
        ExclusiveTab:Toggle({ Title = "Blantant Fishing (Extrem)", Value = false, Callback = function(val) needCast = true; onToggleUB(val) end })
        
        ExclusiveTab:Section({ Title = "Blantant" })
        ExclusiveTab:Input({
            Title = "Delay (detik)", Placeholder = "2.7881", Value = "2.7881",
            Callback = function(text)
                local num = tonumber(text); if not num or num < 1 then NotifyError("Delay", "Minimal 1 detik!"); return end
                Config.YTTA.Settings.QHDelay = math.clamp(num, 1, 10); NotifySuccess("Delay", "Set ke " .. Config.YTTA.Settings.QHDelay .. "s")
            end
        })
        ExclusiveTab:Toggle({ Title = "amBlantat (BETA)", Value = false, Callback = function(val) onToggleYTTA(val) end })
        ExclusiveTab:Slider({ Title = "Set Ub Notif amBlantat", Value = { Min = 1, Max = 10, Default = 3 }, Callback = function(val) Config.YTTA.NotifCount = val end })
        ExclusiveTab:Slider({ Title = "Delay Ub Active (0.01 detik)", Value = { Min = 0, Max = 100, Default = 10 }, Callback = function(val) Config.YTTA.NotifDelay = val / 100 end })

        ExclusiveTab:Toggle({ Title = "Random Cast", Value = false, Callback = function(val) Config.antiOKOK = val end })
        ExclusiveTab:Section({ Title = "Legit Fishing" })
        ExclusiveTab:Input({
            Title = "Delay (detik)", Placeholder = "0.5", Value = "0.5",
            Callback = function(text)
        ...
        ExclusiveTab:Toggle({
            Title = "Legit Fishing", Value = false,
            Callback = function(val)
        ...
        ExclusiveTab:Section({ Title = "Auto Sell" })
        ...
        ExclusiveTab:Toggle({
            ...
            Callback = function(val)
                ...
                ExclusiveTab:Section({ Title = "Totem" })
        ...
        ExclusiveTab:Dropdown({ Title = "Choose Totem", Values = totemValues, Value = totemValues[1], Multi = false, Callback = function(val) Config.SelectedTotemID = totemData[val.Title] or 0; NotifyInfo("Totem", "Sekarang: " .. val.Title) end })

        ExclusiveTab:Toggle({
            Title = "Auto Spawn Totem", Desc = "Spawn otomatis dengan cooldown 1 jam", Value = false,
        ...
        ExclusiveTab:Section({ Title = "Automatic Mix 3 Totem [BETA]" })
        ...
        ExclusiveTab:Toggle({
            Title = "Auto Mix 3 Totem [BETA]", Desc = "Bypass Mode", Value = false,
            Callback = function(v)
                    ...
                    Tasks.mixTotemThread = task.spawn(function()
                        ...
                        while Config.AutoMixTotem do
                            ...
                            for i, config in ipairs(MIX_CONFIG) do
                                    ...
                                    p.Transparency = 1 -- make baseplate invisible
                                    p.CanCollide = false
        ...
        MainTab:Button({
            ...
            Callback = function()
                ...
                for i = 1, 2 do
                    pcall(function()
                        if Events.equipItem then Events.equipItem:FireServer(_G.axeUuid, "Gears") end
                        task.wait(0.5)
                    end)
                    task.wait(3)
                end
            ...
            Callback = function(state)
                ...
                if state then
                    ...
                    if not foundAxe then
                        NotifyWarning("Mining", "Axe tidak ditemukan, silakan beli Axe dulu.")
                    else
                        pcall(function()
                            if Events.equipItem then Events.equipItem:FireServer(_G.axeUuid, "Gears") end
                        end)
                        NotifySuccess("Mining", "Siap! Axe sudah di-equip otomatis.")
                    end
                else
...
if CraftAbilityTab then
    pcall(function()
        ...
        CraftAbilityTab:Input({
            Title = "Delay (detik)", Placeholder = "1.5", Value = "1.5",
            Callback = function(text)
        ...
        CraftAbilityTab:Input({
            Title = "Recipe ID", Placeholder = "contoh: 1", Value = "",
            Callback = function(text)
                local num = tonumber(text)
...
-- AUTO AQUARIUM MANAGER LOOP
        local function StartAutoAquariumManager()
            ...
            AquaMgr.Thread = task.spawn(function()
                while AquaMgr.AutoManage do
                    ...
                    if not ok then
                        warn("AutoAquarium error: " .. tostring(err))
                    end
        ...
        AquariumTab:Section({ Title = "Aquarium" })

        AquariumTab:Button({
            Title = "Open  to Aquarium",
            Desc = "Open Aquarium",
            Callback = function()
                task.spawn(function()
...
-- PILIH TANK
        AquariumTab:Section({ Title = "Tank Aquarium" })
        ...
        AquariumTab:Dropdown({
            Title = "Pilih Tank",
            Desc = "Pilih tank yang ingin di Pakai",
            Values = tankValues,
            Value = tankValues[1],
            Multi = false,
            Callback = function(val)
...
-- ADD IKAN KE TANK
        AquariumTab:Section({ Title = "Tambah Fish ke Tank" })
        ...
        local fishDropdownRef = AquariumTab:Dropdown({
            Title = "Pilih Ikan",
            Desc = "Pilih ikan dari inventory",
        ...
        AquariumTab:Button({
            ...
            Desc = "Ambil ikan terbaru",
            Callback = function()
                local fishList = GetFishList()
                if #fishList == 0 then
                    NotifyWarning("Aquarium", "Tidak ada ikan di inventory!")
                    return
                end
        ...
        AquariumTab:Button({
            ...
            Desc = "Tambah ikan yang dipilih ke tank yang dipilih",
            Callback = function()
        ...
        AquariumTab:Section({ Title = "Quick Tambah per Tank" })
        ...
        -- HAPUS IKAN
        AquariumTab:Section({ Title = "Remove Ikan dari Tank" })
        ...
        -- AUTO SETTINGS
        AquariumTab:Section({ Title = "Auto Set Publish" })
        ...
        -- VISIT
        AquariumTab:Section({ Title = "Aquarium Vist" })
...
-- SECTION: FOUNDER EGG (PAID EGG)
        EventTab:Section({ Title = "Founder Egg" })

        EventTab:Button({
            Title = "Teleport ke Founder Egg",
            Desc = "Teleport ke lokasi Founder Egg di Esoteric Depths",
            Callback = function()
                local hrp = getHRP()
                if hrp then
...
-- SECTION: EGG SYSTEM
        EventTab:Section({ Title = "Egg Setting" })
...
-- SECTION: PET MANAGEMENT
        EventTab:Section({ Title = "Pet Setting" })
...
if MiscTab then
    pcall(function()
        ...
        MiscTab:Toggle({
            ...
            Callback = function(val)
                ...
                if val then
                    ...
                    NotifySuccess("Ultra FPS", "FPS maksimal!")
                else
        ...
        MiscTab:Toggle({
            Title = "Clean", Value = false,
            Callback = function(val)
                if val then
                    -- Destroy all UI and keep track of what was destroyed
                    for _, gui in pairs(LocalPlayer.PlayerGui:GetChildren()) do
                        if gui:IsA("ScreenGui") or gui:IsA("BillboardGui") or gui:IsA("SurfaceGui") then
                            if not _cleanScreenBackup[gui] then
                                _cleanScreenBackup[gui] = {parent = gui.Parent, visible = gui.Enabled}
                            end
                            gui:Destroy()
                        end
                    end
                    ...
                    NotifySuccess("Clean Screen", "Semua UI destroyed! Press again to restore.")
                else
                    -- Attempt to restore destroyed UI by recreating them if possible
                    -- Note: Once destroyed, UI cannot be fully restored, so this is best-effort
                    NotifySuccess("Clean Screen", "UI restoration attempted. Restart for full restore.")
                    _cleanScreenBackup = {}
                end
            end
        })
        ...
        MiscTab:Button({ Title = "Server Hob/AntiStaff", Callback = function()
...
if TradeTab then
    pcall(function()
        ...
        for _, name in ipairs(tradePlayerList) do 
            table.insert(tradePlayerValues, { Title = name, Icon = "user" }) 
        end
        if #tradePlayerValues == 0 then 
            table.insert(tradePlayerValues, { Title = "Tidak ada player lain", Icon = "user-x" }) 
        end
        
        local playerDropdown = TradeTab:Dropdown({ 
            Title = "Target Player", 
            Values = tradePlayerValues, 
            Value = tradePlayerValues[1], 
            Multi = false, 
            Callback = function(val) 
                TradeConfig.SelectedTargetPlayer = val.Title 
                NotifyInfo("Trade", "Target: " .. val.Title)
            end 
        })
        ...
        TradeTab:Button({
            ...
            Callback = function()
                local updated = GetTradePlayerList()
                local newValues = {}
                for _, name in ipairs(updated) do 
                    table.insert(newValues, { Title = name, Icon = "user" }) 
                end
                if #newValues == 0 then 
                    table.insert(newValues, { Title = "Tidak ada player lain", Icon = "user-x" }) 
                end
                pcall(function()
                    if playerDropdown and playerDropdown.Refresh then
                        playerDropdown:Refresh(newValues, newValues[1])
                    end
                end)
                NotifySuccess("Trade", "Players refreshed!")
            end
        })
        ...
        TradeTab:Toggle({
            ...
            Callback = function(val)
                ...
                if val then 
                    SetupAutoAcceptTrade() 
                    NotifySuccess("Auto Accept", "Diaktifkan!")
                else 
                    if autoAcceptConn then 
                        pcall(function() autoAcceptConn:Disconnect() end)
                        autoAcceptConn = nil 
                    end
                    NotifyInfo("Auto Accept", "Dimatikan!")
                end
            end
        })
        ...
        TradeTab:Dropdown({ 
            Title = "Stone Type", 
            Values = stoneTypeValuesT, 
            Value = stoneTypeValuesT[1], 
            Multi = false, 
            Callback = function(val) 
                TradeConfig.SelectedStoneType = val.Title 
            end 
        })
        TradeTab:Input({ 
            Title = "Amount", 
            ...
            Callback = function(text) 
                local num = tonumber(text)
                if num then 
                    TradeConfig.EnchantStoneAmount = math.clamp(num, 1, 50) 
                else
                    NotifyWarning("Trade", "Please enter valid number")
                end 
            end 
        })
        TradeTab:Toggle({ 
            ...
            Value = false, 
            Callback = function(val) 
                TradeConfig.TradeEnchantStone = val
                if val then 
                    StartTradeEnchantStone() 
                else
                    NotifyInfo("Trade", "EnchantStone trade stopped")
                end
            end 
        })
        ...
        TradeTab:Dropdown({ 
            Title = "Select Rarity", 
            Values = tradeRarityValues, 
            Value = tradeRarityValues[1], 
            Multi = false, 
            Callback = function(val) 
                TradeConfig.SelectedTradeRarity = val.Title 
            end 
        })
        TradeTab:Input({ 
            Title = "Amount", 
            ...
            Callback = function(text) 
                local num = tonumber(text)
                if num then 
                    TradeConfig.TradeRarityAmount = math.clamp(num, 1, 50) 
                else
                    NotifyWarning("Trade", "Please enter valid number")
                end 
            end 
        ...
        TradeTab:Toggle({ 
            ...
            Value = false, 
            Callback = function(val) 
                TradeConfig.TradeByRarity = val
                if val then 
                    StartTradeByRarity() 
                else
                    NotifyInfo("Trade", "Rarity trade stopped")
                end
            end 
        })
        ...
        TradeTab:Input({ 
            ...
            Callback = function(text) 
                local num = tonumber(text)
                if num then 
                    TradeConfig.TargetCoins = math.clamp(num, 0, 999999999) 
                else
                    NotifyWarning("Trade", "Please enter valid number")
                end 
            end 
        ...
        TradeTab:Toggle({ 
            ...
            Value = false, 
            Callback = function(val) 
                TradeConfig.TradeByCoin = val
                if val then 
                    StartTradeByCoin() 
                else
                    NotifyInfo("Trade", "Coin trade stopped")
                end
            end 
        })
        ...
        local tradeNameDropdown = TradeTab:Dropdown({ 
            Title = "Select Item", 
            Values = fishItemValues, 
            Value = fishItemValues[1], 
            Multi = false, 
            Callback = function(val)
                if val.Title ~= "Refresh dulu" and val.Title ~= "Inventory kosong" then
                    ...
                    for _, grp in ipairs(_G.TradeGroupedItems) do
                        if grp.Display == val.Title then
                            TradeConfig.SelectedTradeItemUUID = grp.Items and grp.Items[1] or nil
                            break
                        end
                    end
                    NotifyInfo("Trade", "Item: " .. val.Title)
                end
            end 
        })
        ...
        TradeTab:Button({ 
            ...
            Callback = function()
                ...
                for _, item in ipairs(rawItems) do
                    if IsFishItem(item) then 
                        table.insert(fishItems, item) 
                    end
                end
                
                if #fishItems == 0 then
                    NotifyWarning("Trade", "Tidak ada ikan di inventory!")
                    return
                end
                ...
                if #newValues == 0 then 
                    table.insert(newValues, { Title = "Inventory kosong", Icon = "x" }) 
                end
        ...
        TradeTab:Input({ 
            ...
            Callback = function(text) 
                local num = tonumber(text)
                if num then 
                    TradeConfig.TradeNameAmount = math.clamp(num, 1, 50) 
                else
                    NotifyWarning("Trade", "Please enter valid number")
                end 
            end 
        ...
        TradeTab:Toggle({ 
            ...
            Value = false, 
            Callback = function(val) 
                TradeConfig.TradeByName = val
                if val then 
                    StartTradeByName() 
                else
                    NotifyInfo("Trade", "Name trade stopped")
                end
            end 
        })
        ...
        TradeTab:Button({ 
            ...
            Callback = function()
                ...
                TradeConfig.TradeByCoin = false
                TradeConfig.TradeByName = false
                TradeConfig.AutoAcceptTrade = false
                if autoAcceptConn then 
                    pcall(function() autoAcceptConn:Disconnect() end)
                    autoAcceptConn = nil 
                end
