...
-- TABS
local InfoTab      = SafeCreate(function() return Window:Tab({ Title = "Info",       Icon = "rbxassetid://134481698719699" })          end, "InfoTab")
local PlayersTab   = SafeCreate(function() return Window:Tab({ Title = "Player",    Icon = "rbxassetid://134481698719699" })          end, "PlayersTab")
local MainTab      = SafeCreate(function() return Window:Tab({ Title = "Automaly", Icon = "rbxassetid://134481698719699" })           end, "MainTab")
local ExclusiveTab = SafeCreate(function() return Window:Tab({ Title = "Auto Fishing", Icon = "rbxassetid://134481698719699" })         end, "ExclusiveTab")
local CraftAbilityTab = SafeCreate(function() return Window:Tab({ Title = "Craft", Icon = "rbxassetid://134481698719699" }) end, "CraftAbilityTab")
local AquariumTab  = SafeCreate(function() return Window:Tab({ Title = "Aquarium",   Icon = "rbxassetid://134481698719699" })     end, "AquariumTab")
local TeleportTab  = SafeCreate(function() return Window:Tab({ Title = "Teleport",   Icon = "rbxassetid://134481698719699" })      end, "TeleportTab")
local ShopTab      = SafeCreate(function() return Window:Tab({ Title = "Shop",       Icon = "rbxassetid://134481698719699" }) end, "ShopTab")
local EventTab     = SafeCreate(function() return Window:Tab({ Title = "Event",      Icon = "rbxassetid://134481698719699" })     end, "EventTab")
local MiscTab      = SafeCreate(function() return Window:Tab({ Title = "Misc",       Icon = "rbxassetid://134481698719699" })     end, "MiscTab")
local TradeTab     = SafeCreate(function() return Window:Tab({ Title = "Trade",      Icon = "rbxassetid://134481698719699" })       end, "TradeTab")
-- SERVICES
...
local function LoadTradeRemotes()
    ...
    print("Trade Remotes: " .. loaded .. " loaded, " .. failed .. " failed")
end
...
local function StartTradeEnchantStone()
    ...
    task.spawn(function()
        ...
        while TradeConfig.TradeEnchantStone do
            ...
            if not ok then 
                warn("TradeEnchantStone error: " .. tostring(err)) 
...
local function StartTradeByRarity()
    ...
    task.spawn(function()
        ...
        while TradeConfig.TradeByRarity do
            ...
            if not ok then 
                warn("TradeByRarity error: " .. tostring(err)) 
...
local function StartTradeByCoin()
    ...
    task.spawn(function()
        ...
        while TradeConfig.TradeByCoin do
            ...
            if not ok then 
                warn("TradeByCoin error: " .. tostring(err)) 
...
if InfoTab then
    pcall(function()
        InfoTab:Section({ Title = "Informasi" })
        InfoTab:Button({
            Title = "Discord Server", Desc = "copy link",
            Callback = function()
        ...
        InfoTab:Label({ Title = "MNA HUB V0.5 | SCRIPT FREE (Tidak Di Jual Belikan!!!)" })
    end)
end
-- PLAYERS TAB
if PlayersTab then
    pcall(function()
        ...
        local auraDropdownValues = {{ Title = "-- Refresh dulu --", Icon = "refresh-cw" }}
        auraDropdownRef = PlayersTab:Dropdown({
            Title = "Pilih Aura",
            Desc = "Refresh untuk scan aura dari game",
        ...
        PlayersTab:Button({
            Title = "Apply Aura",
            Desc = "Pasang aura yang dipilih Ke Avatar",
            Callback = function()
...
if MainTab then
    pcall(function()
        ...
        MainTab:Dropdown({ Title = "Target (Basic Stones)", Values = basicEnchantValues, Value = basicEnchantValues[1], Multi = false, Callback = function(val) _G.TargetEnchantBasic = val.Title end })
        ...
        MainTab:Dropdown({ Title = "Target (Evolved Stones)", Values = evolvedEnchantValues, Value = evolvedEnchantValues[1], Multi = false, Callback = function(val) _G.TargetEnchantEvolved = val.Title end })
...
if ExclusiveTab then
    pcall(function()
        ExclusiveTab:Section({ Title = "Fishing Ultra" })
        ExclusiveTab:Input({
            Title = "Complete Delay (detik)", Placeholder = "2.798", Value = "2.798",
        ...
        ExclusiveTab:Toggle({ Title = "Fishing [Beta]", Value = false, Callback = function(val) needCast = true; onToggleUB(val) end })
        
        ExclusiveTab:Section({ Title = "Fishing (Extreme)" })
        ExclusiveTab:Input({
            Title = "Delay (detik)", Placeholder = "0.3", Value = "0.3",
            Callback = function(text)
                local num = tonumber(text); if not num or num < 1 then NotifyError("Delay", "Minimal 0.5 detik!"); return end
                Config.YTTA.Settings.QHDelay = math.clamp(num, 1, 10); NotifySuccess("Delay", "Set ke " .. Config.YTTA.Settings.QHDelay .. "s")
            end
        })
        ExclusiveTab:Toggle({ Title = "Sett (Extreme)", Value = false, Callback = function(val) onToggleYTTA(val) end })
        ExclusiveTab:Slider({ Title = "Ub Setting(loop)", Value = { Min = 1, Max = 10, Default = 3 }, Callback = function(val) Config.YTTA.NotifCount = val end })
        ...
        ExclusiveTab:Input({
            Title = "Catch Delay (detik)", Placeholder = "0.5", Value = "0.5",
            Callback = function(text)
        ...
        ExclusiveTab:Section({ Title = "Auto Mix 3 Totem [BETA]" })
        ...
        ExclusiveTab:Toggle({
            Title = "Mix 3 Totem", Desc = "Bypass System", Value = false,
            Callback = function(v)
...
if CraftAbilityTab then
    pcall(function()
        ...
        CraftAbilityTab:Input({
            Title = "Craft Delay", Placeholder = "1.5", Value = "1.5",
            Callback = function(text)
        ...
        CraftAbilityTab:Input({
            Title = "Recipe ID (kosongkan = Opsional)", Placeholder = "contoh: 1", Value = "",
            Callback = function(text)
                local num = tonumber(text)
        ...
        CraftAbilityTab:Toggle({
            Title = "Auto Crafting", Desc = "Craft item otomatis", Value = false,
            Callback = function(val)
...
-- AUTO AQUARIUM MANAGER LOOP
        ...
        AquariumTab:Section({ Title = "Aquarium Management" })
...
-- PILIH TANK
        AquariumTab:Section({ Title = "List Tank" })
        ...
        AquariumTab:Button({
            Title = "Open UI Tank",
            Desc = "Buka UI tank tanpa perlu ke lokasi",
            Callback = function()
                OpenTankUI(AquaMgr.SelectedTank)
            end
        })
-- ADD IKAN KE TANK
        AquariumTab:Section({ Title = "Add Fish Tou Tank" })
        ...
        AquariumTab:Section({ Title = "Tambah Quick Tank" })
        ...
        -- HAPUS IKAN
        AquariumTab:Section({ Title = "Remove Fish" })
        ...
        AquariumTab:Toggle({
            Title = "Fill Semua Tank",
            Desc = "Isi Tank otomatis",
            Value = false,
            Callback = function(val)
                ...
                if val then
                    task.spawn(function()
                        while Config.AquariumAutoFill do
                            ...
                            if #fishList == 0 then
                                NotifyWarning("Aquarium", "No Fish In Inventory")
                                task.wait(30)
                                continue
                            end
...
-- SECTION: FOUNDER EGG (PAID EGG)
        ...
        EventTab:Toggle({
            ...
            Desc = "Auto teleport dan beli Founder Egg",
            Value = false,
            Callback = function(val)
        ...
        EventTab:Button({
            Title = "Buy FounderEgg",
            Desc = "Beli Founder Egg Yang ada",
            Callback = function()
                local hrp = getHRP()
                if hrp then
                    hrp.CFrame = CFrame.new(-4295.189453125, 23.265548706054688, 649.7577514648438, -0.6710231304168701, -7.244620547908198e-08, 0.7414363622665405, -1.815789296699677e-08, 1, 8.127715744876696e-08, -0.7414363622665405, 4.1075931989098535e-08, -0.6710231304168701)
...
if MiscTab then
    pcall(function()
        ...
        MiscTab:Button({ Title = "Server Hop/AntStaff", Callback = function()
...
if TradeTab then
    pcall(function()
        TradeTab:Section({ Title = "pilih Player" })
        ...
        TradeTab:Section({ Title = "By Rarity" })
        ...
        TradeTab:Section({ Title = "By Coin" })
        TradeTab:Input({ 
            Title = "Target Coins", 
            Placeholder = "0", 
            Value = "0", 
        ...
        TradeTab:Section({ Title = "By Name" })
